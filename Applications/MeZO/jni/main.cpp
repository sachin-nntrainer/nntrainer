...
// User data structure for dataset callback
struct UserData {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    std::cout << "Usage: " << argv[0]
              << " <train_images.raw> <train_labels.raw> <val_images.raw> "
                 "<val_labels.raw>"
              << std::endl;
  ...
  try {
    ...
    std::string train_label_file = argv[2];
    std::string val_data_file = argv[3];
    std::string val_label_file = argv[4];
    ...
    auto optimizer = ml::train::createOptimizer(
      "MeZO", {"MeZO_learning_rate=0.00069", " MeZO_epsilon=0.021"});
    model->setOptimizer(std::move(optimizer));

    // Set model properties
    model->setProperty({"epochs=5", "batch_size=32"});
