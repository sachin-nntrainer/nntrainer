...
int NeuralNetwork::train_run(
  ...
  auto train_for_iteration =
    [this, stop_cb, stop_user_data](RunStats &stat, DataBuffer &buffer) {
      // To add here opt.getType() == OptimizerType::MeZO
      if (opt->getType() == MeZO::type) {
        ml_logi("train for iteration using MeZO optimizer");
        // mezo_epsilon to be set from MeZO optimizer

        float mezo_epsilon = 0.0f; // Only defined here to check build
        int seed = 0;              // Only defined here to check build
        ...
        // update weights => optimizer.step(args..)
  ...
  auto eval_epoch_end = [this, batch_size, max_acc = 0.0f,
    ...
    if (stat.accuracy > max_acc ||
      ...
      /// @note this is not actually 'the' min loss for whole time but
