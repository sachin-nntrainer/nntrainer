 ...
 *
 * @file   kai_rhs_pack_nxk_qsi4c32pscalef16_qsu4c32s16s0.h
 * @date   8 December 2025
 * @see    https://github.com/ARM-software/kleidiai
 * @see    https://github.com/nntrainer/nntrainer
 * @author Sungsik Kong <ss.kong@samsung.com>
 *
 * @brief  RHS Packing NxK QSI4C32P ScaleF16 Header
 *
...
/// @param[in] kr The number of columns loaded in the single innermost loop of
/// the matmul micro-kernel.
/// @param[in] bl The block length, which defines the number of K values stored
/// in a single block. It must be equivalent to 32.
///
...
/// the int4 values in a N x K matrix, where N is number of rows and K is the
/// number of columns.
///
/// Two int4 K values are stored in one byte. These values are stored in blocks,
/// where each block has it own scale factor. The scale factor is expected to be
/// a f16 value and stored at the beginning of each block. The first byte in the
/// block holds the K-index + 0 and K-index + 16 values. The K-index + 0 value
/// is stored in the lower order part of the byte (low nibble) while the K-index
/// + 16 value is stored in the higher order part (high nibble). For example, if
/// the block length is 32, the values are stored in the following order:
/// |byte(s16, s0),byte(s17, s1),byte(s18, s2),...,byte(s31,
/// s15),float16(scale)|
///
...
/// @param[in] nr       The number of columns written by the matmul micro-kernel
/// @param[in] kr       The number of columns loaded in the single inner most
/// loop of the matmul micro-kernel.
/// @param[in] bl       The block length, which defines the number of K values
/// stored in a single block. It must be a multiple of 32.
///
...
///
/// The int4 values are stored in a N x K matrix, where N is number of rows and
/// K is the number of columns. Two int4 values are stored in one byte. The
/// lower order part of the byte (low) holds the first nibble (K-index + 0). The
...
/// (K).
/// @param[in]  nr          The number of columns written by the matmul
/// micro-kernel.
...
///                         K values stored in a single block. It must be a
///                         multiple of 32.
