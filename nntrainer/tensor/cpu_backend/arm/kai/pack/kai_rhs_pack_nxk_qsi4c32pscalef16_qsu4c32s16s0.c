...
void kai_run_rhs_pack_nxk_qsi4c32pscalef16_qsu4c32s16s0(
  ...
  size_t bl, const uint8_t *rhs, const float *bias, void *rhs_packed,
  size_t extra_bytes, const struct kai_rhs_pack_qs4cxs1s0_param *params) {
  KAI_ASSUME(bl == kai_bl);
  ...
  for (size_t y = 0; y < n; y += nr) {
    ...
    for (size_t x = 0; x < num_blocks_per_row; ++x) {
      ...
      for (size_t i = 0; i < nr; ++i) {
        ...
        memcpy(scales + i * kai_num_bytes_multiplier,
               src_row + src_row_idx * rhs_stride, kai_num_bytes_multiplier);
      }
      ...
      for (size_t s = 0; s < num_segments_per_block; ++s) {
        for (size_t i = 0; i < nr; ++i) {
          ...
          if (num_bytes_per_segment == sizeof(uint32_t)) {
            uint32_t tmp = 0;
            memcpy(&tmp, src_row + src_row_idx * rhs_stride,
                   num_bytes_per_segment);
            tmp = tmp ^ 0x88888888;
            memcpy(dst_row + i * num_bytes_per_segment, &tmp,
                   num_bytes_per_segment);
            ...
            memcpy(dst_row + i * num_bytes_per_segment,
                   src_row + src_row_idx * rhs_stride, num_bytes_per_segment);
